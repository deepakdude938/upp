package com.upp.stepdefinition;

import java.io.IOException;

import com.upp.base.BaseClass;
import com.upp.handlers.DealGroupAttributesHandler;
import com.upp.handlers.TransactionMaker_PaymentInstrumentHandler;
import com.upp.pagemodules.Budget;
import com.upp.pagemodules.DashBoard_Module;
import com.upp.pagemodules.Deal.DealAccountCreator;
import io.cucumber.java.en.Then;

public class TS71 extends BaseClass{
	
	
	
	public Budget budget;
	public String tsid;
	public static String sourceAccountNo = "";
	public static String toAccountNo = "";
	public DashBoard_Module dashboard;


	public TS71() {
		budget = new Budget();
		dashboard = new DashBoard_Module();

	}
	
	@Then("Create a Budget for half yearly purpose  with given {string}")
	public void create_a_Budget_for_half_yearly_purpose_with_given(String string) throws IOException, Exception {
		budget.CreateBudget_Purpose_HalfYearly(string,DealPage.sourceAccountNo,DealPage.toaccountNo);
	}
	
	
}
