...
public class Rule_OBODetails_Null_OBO extends BaseClass {
	...
	public static void Rule_Non_OBO_Api(String dealId, String TSID) throws Exception {
		...
		Response res = given().header("Content-Type", "application/json")
				.header("Authorization", LoginAPI_UPP.authToken).body(Payload.Rule_OBODetails_Null_OBO(dealId, TSID)).when()
				.post("transaction/api/transaction");

		response = res.then().extract().asString();
		System.out.println("Rule_OBODetails_Null_OBO " + response);
